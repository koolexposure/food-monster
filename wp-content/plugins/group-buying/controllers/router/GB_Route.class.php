...
class GB_Route extends GB_Router_Utility {
	protected $id = '';
	protected $path = '';
	...
	public function execute( WP $query ) {
		...
		$page = new GB_Router_Page( $page_contents, $title, $template );
	}
	...
	protected function error_403() {
		$message = apply_filters( 'gb_router_access_denied_message', self::__( 'You are not authorized to access this page' ) );
		$title = apply_filters( 'gb_router_access_denied_title', self::__( 'Access Denied' ) );
		$args = apply_filters( 'gb_router_access_denied_args', array( 'response' => 403 ) );
