<?php
/**
 * GBS Router Controller
 *
 * @package GBS
 * @subpackage Router
 */

class GB_Router extends GB_Router_Utility {
	const ROUTE_CACHE_OPTION = 'GB_Router_route_hash';
	private $routes = array();

	/**
	 *
	 *
	 * @var GB_Router The one instance of this singleton class
	 */
	 ...
	 * Action: init
	 *
	 * @uses do_action() Calls 'gb_router_generate_routes'
	 * @uses do_action() Calls 'gb_router_alter_routes'
	 * @return void
	 */
	public function generate_routes() {
		do_action( 'gb_router_generate_routes', $this );
		do_action( 'gb_router_alter_routes', $this );
	...
	protected function redirect_placeholder( $query ) {
		...
		// the page for a GB Router post, and there's only one of those
		if ( !empty( $query->query_vars[GB_Router_Page::POST_TYPE]) ) {
	...
	protected function identify_route( $query ) {
		...
		if ( !isset( $this->routes[$id] ) || !is_a( $this->routes[$id], 'GB_Route' ) ) {
			return NULL;
		}
		return $id;
	}
	...
	/**
	 * Create a new GB_Route with the given id and properties
	 *
	...
	protected function create_route( $id, array $properties ) {
		try {
			$route = new GB_Route( $id, $properties );
		} catch ( Exception $e ) {
