 ...
 */
class Group_Buying_Account_Balance_Payments extends Group_Buying_Payment_Processors {
	const PAYMENT_METHOD = 'Account Credit (Balance)';
	const RESERVED_CREDIT_META = '_gbs_reserved_account_balance_credit';
	...
	protected function __construct() {
		...
		// force payment method on free items.
		add_action( 'processing_payment', array( $this, 'register_free_payment_method' ), 10, 2 );
	}
	...
	public function register_free_payment_method( Group_Buying_Checkouts $checkout, Group_Buying_Purchase $purchase ) {
		...
		foreach ( $items as $key => $item ) {
			if ( empty($items[$key]['payment_method'][0]) || $items[$key]['payment_method'][0] != $this->get_payment_method() ) {
				if ( $item['price'] < 0.01 ) {
	 ...
	 * for any account credits being used
	 *
	 * @param unknown $total
	...
	public function release_payment( Group_Buying_Payment $payment ) {
		if ( $payment->get_payment_method() == $this->get_payment_method() && $payment->get_status() != Group_Buying_Payment::STATUS_COMPLETE ) {
			...
			if ( $items_to_release ) {
				...
				if ( self::DEBUG ) error_log( '----------Account Balance Credits Release----------' . print_r( $items_to_release, TRUE ) );
