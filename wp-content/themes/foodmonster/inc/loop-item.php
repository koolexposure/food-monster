<li>
<div id="post_content <?php the_ID() ?>" <?php post_class('loop_deal_spec shadow'); ?>>

	<?php if ( has_post_thumbnail() ): ?>
			<a href="<?php the_permalink() ?>" title="<?php gb_e('Read'); ?> <?php the_title() ?>"><div class="loop_thumb"><?php the_post_thumbnail() ?></div></a>
	<?php else : ?>
	...
	<!--	<?php if ( gb_has_merchant() ) {?>
		...
		<div class="meta_value_worth">
			...
			<span class="deal_meta_value deal_worth"><?php gb_formatted_money(gb_get_deal_worth()) ?></span>
		</div>
		<div class="meta_value_saving">
			<span class="deal_meta_title"><?php gb_e('Savings: '); ?></span>
			<span class="deal_meta_value deal_savings"><?php gb_amount_saved() ?></span>
		</div>
